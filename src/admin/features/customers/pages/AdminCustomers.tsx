...
import { getCustomers, getCustomerAppointments, updateCustomerNotes, updateCustomerProfile, deleteCustomer } from '../../../services/customers';
import type { SystemUser, CustomerData } from '../../../../shared/types/user';
...
export const AdminCustomers: React.FC<AdminCustomersProps> = ({ onLogout, systemUser }) => {
  ...
  const fetchCustomerAppointments = useCallback(async (customerId: number) => {
    ...
    try {
      const formatted = await getCustomerAppointments(customerId);
  ...
  const handleSaveNotes = async () => {
    ...
    try {
      await updateCustomerNotes(selectedCustomer.id, notesText.trim() || null);
    ...
    try {
      if (consentModalState.actionType === 'edit') {
        await updateCustomerProfile(
          ...
          editForm.password_hash.trim() || null
        );
