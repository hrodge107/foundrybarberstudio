...
export const AdminReports: React.FC<AdminReportsProps> = ({ onLogout, systemUser }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setAppointments(sorted as unknown as AppointmentRecord[]);
    } catch (err: any) {
  ...
  const customerEarliestMap = useMemo(() => {
    ...
    appointments.forEach((a) => {
      ...
      const d = new Date(a.appointment_date);
      const existing = map.get(a.customer.id);
      if (!existing || d < existing) {
        map.set(a.customer.id, d);
      }
    });
    return map;
  }, [appointments]);
  ...
  const kpis = useMemo(() => {
    ...
    const uniqueClientsCount = new Set(completedCurrent.map((a) => a.customer?.id).filter(Boolean)).size;

    let newClientsCount = 0;
    customerEarliestMap.forEach((earliestDate, customerId) => {
      if (earliestDate >= currentStart && earliestDate <= currentEnd) {
        if (completedCurrent.some((a) => a.customer?.id === customerId)) {
          newClientsCount++;
        }
      }
    });
  ...
  const dailyChartData = useMemo(() => {
    if (period === 'today') {
      const hours = Array.from({ length: 18 }, (_, i) => 6 + i); // 6 AM to 11 AM (23)
      return hours.map((h) => {
        ...
        const label = h === 12 ? '12PM' : h === 23 ? '11PM' : h > 12 ? `${h - 12}PM` : `${h}AM`;
  ...
  // Busiest Hours (6am to 11pm)
  const busiestHours = useMemo(() => {
    const hours = Array.from({ length: 18 }, (_, i) => 6 + i); // 6 to 23
      ...
      <div className="admin-reports-page">
        ...
        {isLoading ? (
          ...
          <div className="reports-body-grid">
            ...
            {/* Daily Appointments Chart (Full Row) */}
            ...
            {/* Split Row: Bookings Status Donut & Revenue by Category */}
                    ...
                    return (
                      <div key={idx} className="dow-col" title={`${d.day}: ${d.count} appointments`}>
                        <div className="dow-bar-track">
                          <div
                            className={`dow-bar-fill ${isWeekend ? 'weekend-highlight' : ''}`}
                            style={{ height: `${Math.max(8, barHeight)}%` }}
                          >
                ...
                <div className="barber-workload-list">
                  {barberPerformance.length === 0 ? (
                    ...
                    barberPerformance.map((b, idx) => {
                      ...
                      // Categorical flat color palette for distinct barbers
                      const barberColors = ['#4f46e5', '#0d9488', '#ea580c', '#db2777', '#2563eb', '#7c3aed'];
                      const barColor = barberColors[idx % barberColors.length];
