...
import { ServiceModal, type ServiceData } from '../components/ServiceModal';
    ...
    try {
      if (editingCategoryName) {
        ...
        const { error } = await supabase
          ...
          .eq('category_name', editingCategoryName);
        if (error) throw error;
        ...
        // New category: select it as active so user can immediately add services under it
        setSelectedCategory(newCategoryName);
    ...
    try {
      if (deleteTarget.type === 'service' && deleteTarget.id) {
        ...
        const { error } = await supabase
          .from('services')
          .delete()
          .eq('category_name', deleteTarget.name);
        if (error) throw error;
