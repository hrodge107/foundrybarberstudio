...
interface BookingTrackerProps {
  onTrackBooking: (bookingId: string) => Promise<{
    id: number;
...
export const BookingTracker: React.FC<BookingTrackerProps> = ({ onTrackBooking }) => {
    ...
    const checkHashAndTrack = async () => {
      ...
      if (targetId) {
        ...
        if (/^FBS-\d{6}$/.test(normalized)) {
          ...
          setSearched(true);
          try {
            const data = await onTrackBooking(normalized);
            setBooking(data);
          } catch (e) {
            console.error(e);
            setBooking(null);
