...
import { getStoreHours } from '../../../services/booking';
...
export const SidebarPlaceholder: React.FC = () => {
  ...
  useEffect(() => {
    const fetchHours = async () => {
      try {
        ...
        console.error('Error fetching store hours in sidebar:', e);
      }
    };
    fetchHours();
  }, []);
  ...
  const statusInfo = useMemo(() => {
    ...
    const currentDayHours = storeHours.find((h: StoreHour) => h.day_of_week === dayAbbr);
    ...
    const getNextOpenText = (startDayOffset = 1): string => {
      for (let i = 0; i < 7; i++) {
        ...
        const checkDay = storeHours.find((h: StoreHour) => h.day_of_week === DAYS_MAP[checkIndex]);
  ...
  const schedule = useMemo(() => {
    return storeHours.map((h: StoreHour, idx: number) => {
