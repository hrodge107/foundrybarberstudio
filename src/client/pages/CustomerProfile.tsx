...
export const CustomerProfile: React.FC<CustomerProfileProps> = ({ customerUser, onNavigate, onUpdateCustomerUser }) => {
  ...
  const fetchCustomerAppointments = async () => {
    ...
    try {
      ...
      const formatted: AppointmentDetail[] = (data || []).map((item: any) => {
        const rawBarber = Array.isArray(item.barber) ? item.barber[0] : item.barber;
        const rawService = Array.isArray(item.service) ? item.service[0] : item.service;
        return {
          id: item.id,
